<?php !defined('BOYAA') AND exit('Access Denied!');

class socketpacket {
	const SERVER_PACEKTVER = 1;
	const SERVER_SUBPACKETVER = 1; //2
	const PACKET_BUFFER_SIZE = 8192;
	const PACKET_HEADER_SIZE = 9;//9
	
	private $m_packetBuffer;
	private $m_packetSize;
	private $m_CmdType;
	private $m_cbCheckCode;
	private $m_version;
	private $m_subVersion;

    //发送字节映射表
    public static $m_SendByteMap;
    //接收字节映射表
    public static $m_RecvByteMap;

	public function __construct(){
		$this->m_packetSize = 0;
		$this->m_packetBuffer = "";
		$this->m_version = self::SERVER_PACEKTVER;
		$this->m_subVersion = self::SERVER_SUBPACKETVER;
	}
	
	public function WriteBegin($CmdType, $version = self::SERVER_PACEKTVER, $subVersion = self::SERVER_SUBPACKETVER){
		$this->m_CmdType = $CmdType;
		$this->m_version = $version;
		$this->m_subVersion = $subVersion;
	}

	public function WriteInt_N($value){
		$this->m_packetBuffer .= pack("N", $value);
		$this->m_packetSize += 4;
	}
	
    public function WriteEnd_n($flag='no_encry')
    {
 		$code = 0;
        if($flag=='encry')
        {
            $code=self::EncryptBuffer();
        }
     
		$content .= pack("n", $this->m_packetSize +7 );			//len
		$content .= "BY";
		$content .= pack("c", $this->m_version);			    //ver
		$content .= pack("c", $this->m_subVersion);			    //subver
		$content .= pack("n", $this->m_CmdType);			    //cmd
		$content .= pack("c", $this->m_cbCheckCode);						    //
		$this->m_packetBuffer = $content . $this->m_packetBuffer;
	}
	
	public function ReadInt_N($format = "N"){
		if(empty($this->m_packetBuffer)){
			fc::debug("buffer empty: $this->m_packetBuffer}, trace:".json_encode(debug_backtrace()), 'socketpacket.txt');
		}
		$temp = substr($this->m_packetBuffer, 0, 4);
		$value = unpack($format, $temp);
		if(false===$value){
			fc::debug("unpack fail, format:{$format}, temp:{$temp}, trace:".json_encode(debug_backtrace()), 'socketpacket.txt');
		}
		$this->m_packetBuffer = substr($this->m_packetBuffer, 4);
		return $value[1];
	}

    private function EncryptBuffer()
    {
        $code=0;
        $trans_data="";
        for( $i=0;$i<$this->m_packetSize;$i++)
        {
            $temp=substr($this->m_packetBuffer, 0, 1);
            $this->m_packetBuffer = substr($this->m_packetBuffer, 1);
            $value = unpack("C", $temp);
            $code+=$value[1];
            $code=$code&0xFF;

            $rev=socketpacket::$m_SendByteMap[$value[1]];
            $trans_data.=pack("C",$rev);
        }
        $this->m_packetBuffer=$trans_data;
        $this->m_cbCheckCode=(~$code)+1;
    }

    private function DecryptBuffer()
    {
        $code=$this->m_cbCheckCode;
        if($code==0)
        {
            return 0;
        }

        $trans_data="";
        for($i=0;$i<$this->m_packetSize;$i++)
        {
            $temp=substr($this->m_packetBuffer, 0, 1);
            $this->m_packetBuffer = substr($this->m_packetBuffer, 1);
            $value = unpack("C", $temp);
            $rev=socketpacket::$m_RecvByteMap[$value[1]];

            $code+=$rev;
            $code=$code&0xFF;
            $trans_data.=pack("C",$rev);
        }

        if($code!=0)
        {
            return -1;
        }

        $this->m_packetBuffer=$trans_data;
        return 0;
    }
	
	public function WriteEnd(){
		//$EncryptObj = new EncryptDecrypt();
 		$code = 0;//$EncryptObj->EncryptBuffer($this->m_packetBuffer, 0, $this->m_packetSize);
     
		$content .= pack("s", $this->m_packetSize +7 );			//len
		$content .= "BY";
		$content .= pack("c", $this->m_version);			    //ver
		$content .= pack("c", $this->m_subVersion);			    //subver
		$content .= pack("s", $this->m_CmdType);			    //cmd
		$content .= pack("c", $code);						    //
		$this->m_packetBuffer = $content . $this->m_packetBuffer;

	}
		
	public function GetPacketBuffer(){
		return $this->m_packetBuffer;
	}
	
	public function GetPacketSize(){
		return $this->m_packetSize + self::PACKET_HEADER_SIZE ;
	}
	
	public function WriteInt($value){
		$this->m_packetBuffer .= pack("i", $value);
		$this->m_packetSize += 4;
	}
	
	public function WriteUInt($value){
		$this->m_packetBuffer .= pack("I", $value);
		$this->m_packetSize += 4;
	}
	
	public function WriteByte($value){
		$this->m_packetBuffer .= pack("C", $value);
		$this->m_packetSize += 1;
	}
	
	public function WriteShort($value, $format = 's'){
		$this->m_packetBuffer .= pack($format, $value);
		$this->m_packetSize += 2;
	}
	
	public function WriteString($value){

		$len = strlen($value)+1;
		$this->m_packetBuffer .= pack('i', $len);
		$this->m_packetBuffer .= $value;
		$this->m_packetBuffer .= pack("C", 0);
		$this->m_packetSize += $len+4;
	}
	
	public function WriteString_N($value){

		$len = strlen($value)+1;
		$this->m_packetBuffer .= pack("N", $len);
		$this->m_packetBuffer .= $value;
		$this->m_packetBuffer .= pack("C", 0);
		$this->m_packetSize += $len+4;
	}

	public function WriteBinary($value,$length){

        $len=$length;
		$this->m_packetBuffer .= pack("N", $len);
		$this->m_packetBuffer .= $value;
		$this->m_packetSize += $len+4;
	}

    public function WritePacket($subPkg){
        $len = $subPkg->GetPacketSize();
		$this->m_packetSize += 4;
        $this->m_packetBuffer .= pack("N", $len);
        for($i=0;$i<$len;$i++)
        {
            $data=$subPkg->ReadByte();
            $this->m_packetBuffer .= pack("C", $data);
        }
		$this->m_packetSize += $len;
    }

    public function GetCmdType(){
        return $this->m_CmdType;
    }
    public function dump_this()
    {
    	var_dump($this->m_cbCheckCode);
    }

    public function ReadPacket($subPkg){
        $len = $this->ReadInt_N();
        $subPkg->SetRecvPacketBuffer($this->m_packetBuffer,$len);
    }
	
	public function ParsePacket(){
		if( $this->m_packetSize < self::PACKET_HEADER_SIZE ){
			return false;
		}

		$header = substr($this->m_packetBuffer, 0, 9);
        $arr= unpack("nLen/c2Iden/cVer/cSubVer/nCmdType/cCode", $header);

		if($arr['Iden1'] != ord('B') || $arr['Iden2'] != ord('Y')){
		    echo "in ParsePacket 4<br/>";
			return -1;
		}
		if ($arr['Ver'] != $this->m_version || $arr['SubVer'] != $this->m_subVersion){
			//return -2;
		}
		if($arr['CmdType'] <= 0 || $arr['CmdType'] >= 32000){
			return -3;
		}
		if($arr['Len'] >= 0 && $arr['Len'] > self::PACKET_BUFFER_SIZE - self::PACKET_HEADER_SIZE ){
			return -4;
		}
	 	$this->m_cbCheckCode  = $arr['Code'];
		$this->m_packetBuffer = substr($this->m_packetBuffer, 9);

		return 0;
	}
	
	public function SetRecvPacketBuffer($packet_buff, $packet_size){
        $header= substr($packet_buff,0,self::PACKET_HEADER_SIZE);
        $varr = unpack("nLen/c2Iden/cVer/cSubVer/nCmdType/cCode", $header);
        $this->m_CmdType=$varr['CmdType'];
        $this->m_cbCheckCode=$varr['Code'];
        $this->m_version=$varr['Ver'];
        $this->m_subVersion=$varr['SubVer'];
		$this->m_packetBuffer = substr($packet_buff,self::PACKET_HEADER_SIZE);
		$this->m_packetSize  = $packet_size-self::PACKET_HEADER_SIZE;

        if($this->m_cbCheckCode!=0 && self::DecryptBuffer()<0)
        {
        	fc::debug('recv buffer, BY Protocol DecryptBuffer Error', 'socketpacket.txt');
            return -1;
        }

        return 1;
	}

	public function ReadInt(){
		$temp = substr($this->m_packetBuffer, 0, 4);
		$value = unpack("i", $temp);
		$this->m_packetBuffer = substr($this->m_packetBuffer, 4);
		return $value[1];
	}
	
	public function ReadUInt(){
		$temp = substr($this->m_packetBuffer, 0, 4);
		list(,$var_unsigned)=unpack("L",$temp);
		return floatval(sprintf("%u",$var_unsigned));
	}

	public function ReadShort($format = 's'){
		$temp = substr($this->m_packetBuffer, 0, 2);
		$value = unpack($format, $temp);
		$this->m_packetBuffer = substr($this->m_packetBuffer, 2);
		return $value[1];
	}
	
	public function ReadString(){
		$len = $this->ReadInt_N();
		$value = substr($this->m_packetBuffer, 0, $len-1);
		$this->m_packetBuffer = substr($this->m_packetBuffer, $len);
		return $value;
	}

	public function ReadBinary(){
		$len = $this->ReadInt_N();
		$value = substr($this->m_packetBuffer, 0, $len);
		$this->m_packetBuffer = substr($this->m_packetBuffer, $len);
		return $value;
	}
	
	public function ReadByte(){
		$temp = substr($this->m_packetBuffer, 0, 1);
		$value = unpack("C", $temp);
		$this->m_packetBuffer = substr($this->m_packetBuffer, 1);
		return $value[1];
	}
}//end-class

//发送字节映射表
socketpacket::$m_SendByteMap=	array(			
    
);

//接收字节映射表
socketpacket::$m_RecvByteMap=array(				 
    
);

